...
if(isset($_POST['absences'])){
  ...
  $regot = $_POST['regot'];
  $restot = $_POST['restot'];
  $nightdiff = $_POST['nightdiff'];
  $regholiday = $_POST['regholiday'];
  $specialholiday = $_POST['specialholiday'];
  $salaryadjustments = $_POST['salaryadjustments'];
  $month = ltrim($_POST['month'], '0');
  $year = $_POST['year'];
  $batch = $_POST['batch'];
  ...
  $absencesrate = hourlyRate($_POST['absences'], $basicpay);
  $tardinessrate = minuteRate($_POST['tardiness'], $basicpay);
  $undertimerate = minuteRate($_POST['undertime'], $basicpay);
  $regotrate = hourlyRate($_POST['regot'], $basicpay) * 1.25;
  $restotrate = hourlyRate($_POST['restot'], $basicpay) * 1.30;
  $nightdiffrate = hourlyRate($_POST['nightdiff'], $basicpay) * 0.1;
  $regholidayrate = hourlyRate($_POST['regholiday'], $basicpay);
  $specialholidayrate = hourlyRate($_POST['specialholiday'], $basicpay);

  $grosspay = ($basicpay / 2) - $absencesrate - $tardinessrate - $undertimerate + $regotrate + $restotrate + $nightdiffrate + $regholidayrate + $specialholidayrate;
  $grosspay = round($grosspay, 2) + $deminimis;


    $statement = $connection->prepare('
    START TRANSACTION;
    INSERT INTO grosspay (GrossPayId, EmployeeId, Month, Year, Batch, BasicPay, Absences, Tardiness, Undertime, RegOT, RestOT, NightDifferentials, RegHoliday, SpecialHoliday, SalaryAdjustments, GrossPay)
    VALUES (NULL, :EmployeeId, :Month, :Year, :Batch,  :BasicPay, :Absences, :Tardiness, :Undertime, :RegOT, :RestOT, :NightDifferentials, :RegHoliday, :SpecialHoliday, :SalaryAdjustments, :GrossPay);
    INSERT INTO notifications (id, EmployeeId, Type)
    VALUES (NULL, :EmployeeId, 0);
    UPDATE users SET hasDTR = 1 WHERE EmployeeId = :EmployeeId;
    COMMIT;');
    ...
    $result = $statement->execute(array(
      ...
      if ($result) {
        $result = $mysqli->query("SELECT HDMF, SSS, OtherDeduction FROM loans WHERE EmployeeId = '$id' AND Month = '$month' AND Year = '$year' AND Batch = '$batch'");
        if ($result->num_rows != 0) {
          ...
          if ($batch == 2) {
            $cutoff = '2ND CUT-OFF';
            $datebasis = date('F d', strtotime(get_date('from'))) .' - '. date('d, Y', strtotime(get_date('to')));
          }else {
            $cutoff = '1ST CUT-OFF';
            $datebasis = date('F d', strtotime(get_date('from'))) .' - '. date('d, Y', strtotime(get_date('to')));
          }
